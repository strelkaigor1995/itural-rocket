			...
			default:
			  $country_data = $wpdb->get_row("SELECT * FROM `".WPSC_TABLE_CURRENCY_LIST."` WHERE `isocode` IN('".$_SESSION['wpsc_delivery_country']."') LIMIT 1",ARRAY_A);
				...
				$output = "<input title='".$this->checkout_item->unique_name."' type='hidden' id='".$this->form_element_id()."' class='shipping_region' name='collected_data[{$this->checkout_item->id}]' value='".$_SESSION['wpsc_delivery_region']."' size='4' /><span class='shipping_region_name'>".$region_name."</span> ";
					...
					$output = "<input title='".$this->checkout_item->unique_name."' type='text' id='".$this->form_element_id()."' class='text' value='yo".$saved_form_data."' name='collected_data[{$this->checkout_item->id}]".$an_array."' />";
...
*/
   ...
   if(isset($_POST['cctype'])){
	...
	// jakal, no validate form now, just save
	/*if($user_ID > 0) {
	...
	// jakal, replace code above with codex update_user_meta
	...
	* @access public
	*/
  function save_forms_to_db($purchase_id, $collected_data) {
		...
		//echo '<pre>'.print_r($_POST['collected_data'], true).'</pre>';
		//echo '<pre>'.print_r($this->checkout_items, true).'</pre>';
		...
		foreach($this->checkout_items as $form_data) {
		  ...
		  //$value = $_POST['collected_data'][$form_data->id];
		  $value = $collected_data[$form_data->id];
		  if($value == ''){
