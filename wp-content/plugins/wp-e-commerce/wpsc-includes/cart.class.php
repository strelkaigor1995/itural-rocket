...
// jakal, total without shipping
function wpsc_cart_subtotal($forDisplay = true) {
	global $wpsc_cart; 
	$output = $wpsc_cart->calculate_subtotal();
	if($forDisplay){
		$output = $wpsc_cart->process_as_currency($output); 
	}
	return $output;
}
...
*/
function wpsc_cart_shipping($display=true) {
	global $wpsc_cart;
	if($display){
		$output = $wpsc_cart->process_as_currency($wpsc_cart->calculate_total_shipping());
	} else {
		$output = $wpsc_cart->calculate_total_shipping();
	}
	return $output;
}
...
class wpsc_cart {
	...
	// jakal
	//var $my_shipping_destination_id = 0;
	var $my_shipping_destination = array();
	var $my_location = array();
	...
	function wpsc_cart() {
		...
		if(!defined('DEVELOPMENT')){
			$dadata_location = my_get_location_by_ip($_SERVER['REMOTE_ADDR']);
		} else { // для отладки используем Пышму
			$dadata_location = my_get_location_by_ip('46.160.149.249'); // Верхняя Пышма
		}
		
		//exit('wpsc_cart' . var_dump($dadata_location));
				
		//$dadata_location = my_get_location_by_ip('85.12.194.71'); // hq
		//$dadata_location = my_get_location_by_ip('31.13.144.49'); // Самара
		//$dadata_location = my_get_location_by_ip('178.211.173.107'); // Ревда
		//$dadata_location = my_get_location_by_ip('37.113.134.69'); // Челябинск
		//$dadata_location = my_get_location_by_ip('217.20.80.9'); // Ханты-Мансийск
		//$dadata_location = my_get_location_by_ip('88.205.245.85'); // Салехард
		//$dadata_location = my_get_location_by_ip('46.160.149.249'); // Верхняя Пышма
		
		if($dadata_location !== false && isset($dadata_location->location)){
			
			$this->my_location['region_fias_id'] = isset($dadata_location->location->data->region_fias_id)?$dadata_location->location->data->region_fias_id:null;
			$this->my_location['region_with_type'] = isset($dadata_location->location->data->region_with_type)?$dadata_location->location->data->region_with_type:null;
			$this->my_location['area_fias_id'] = isset($dadata_location->location->data->area_fias_id)?$dadata_location->location->data->area_fias_id:null;
			$this->my_location['area_with_type'] = isset($dadata_location->location->data->area_with_type)?$dadata_location->location->data->area_with_type:null;
			$this->my_location['city_fias_id'] = isset($dadata_location->location->data->city_fias_id)?$dadata_location->location->data->city_fias_id:null;
			$this->my_location['city_with_type'] = isset($dadata_location->location->data->city_with_type)?$dadata_location->location->data->city_with_type:null;			
			$this->my_location['settlement_fias_id'] = isset($dadata_location->location->data->settlement_fias_id)?$dadata_location->location->data->settlement_fias_id:null;
			$this->my_location['settlement_with_type'] = isset($dadata_location->location->data->settlement_with_type)?$dadata_location->location->data->settlement_with_type:null;
			
			$destination_id = 5; // <Другой город> по-умолчанию, если есть гео
			
			// ищем соответствие гео в нашей базе регионов и переопределяем destination_id
			$dest_arr_arr = my_get_destinations();
			foreach($dest_arr_arr as $dest_arr){
				$fias_type = $dest_arr['fias_type'];
				$fias_id = $dest_arr['fias_id'];
				if(!empty($fias_id) && $this->my_location[$fias_type] === $fias_id){
					$destination_id = $dest_arr['id'];
					break;
				}
			}
			...
			$this->my_location['region_fias_id'] = null;
			$this->my_location['region_with_type'] = null;
			$this->my_location['area_fias_id'] = null; 
			$this->my_location['area_with_type'] = null;
			$this->my_location['city_fias_id'] = null;
			$this->my_location['city_with_type'] = null;
			$this->my_location['settlement_fias_id'] = null;
			$this->my_location['settlement_with_type'] = null;		
			
			$destination_id = 1; // если нет информации о гео, то ставим по-умолчанию Екатеринбург
			
		}		
		
		// если залогинен и сохранен регион, то используем его
		// то же самое при логине делаем, через экш в функциях.пхп
		get_currentuserinfo();
		$meta_data = get_user_meta($current_user->ID, 'wpshpcrt_usr_profile', true);
		if(!empty($meta_data)){
			if(isset($meta_data[42]) && is_numeric($meta_data[42])){ // my_destination_id (скрытый параметр)
				$destination_id = $meta_data[42];
			}				
		}
		
		$this->get_shipping_method(); // выбираем my_shipping как метод доставки
		$this->update_location($destination_id);
	...
	function my_destination_onchange(){
		
		global $wpsc_shipping_modules;
		
		//exit(var_dump($wpsc_shipping_modules));
		//exit(var_dump($this->selected_shipping_method));
		//exit(var_dump($wpsc_shipping_modules[$this->selected_shipping_method]));
		//exit(var_dump($this->my_shipping_destination['id']));
		
		$result_arr = my_get_destinations($this->my_shipping_destination['id']);
		
		if($result_arr === null){
			$this->my_shipping_destination['name'] = null;
			$this->my_shipping_destination['fias_type'] = null;
			$this->my_shipping_destination['fias_id'] = null;			
		} else {
			$this->my_shipping_destination['name'] = $result_arr['name'];
			$this->my_shipping_destination['fias_type'] = $result_arr['fias_type'];
			$this->my_shipping_destination['fias_id'] = $result_arr['fias_id'];
		}

		$result_arr = $wpsc_shipping_modules[$this->selected_shipping_method]->getShippingMethods($this->my_shipping_destination['id']);
	
		if($result_arr === null){
			$this->my_shipping_destination['public_name'] = null;
			$this->my_shipping_destination['internal_service'] = null;
		} else {
			$this->my_shipping_destination['public_name'] = $result_arr['name'];
			$this->my_shipping_destination['internal_service'] = $result_arr['internal_service'];
		}

		$this->my_shipping_destination['min_cart_for_delivery'] = 
			$wpsc_shipping_modules[$this->selected_shipping_method]->getMinCartForDelivery($this->my_shipping_destination['id']);
		
		$this->my_shipping_destination['min_cart_for_free_delivery'] = 
			$wpsc_shipping_modules[$this->selected_shipping_method]->getMinCartForFreeDelivery($this->my_shipping_destination['id']);				
  ...
  /**
	...
	* @access public
	*/
	function update_location($destination_id = 1) {
		...
		*/
		...
		$destination = my_get_destinations($destination_id);
		if($destination === null) $destination_id = 1; // если нет в базе, то ставим 1
		
		//var_dump($destination_id);

		$this->my_shipping_destination['id'] = $destination_id;

		$this->my_destination_onchange();
		$this->clear_cache(); // reset cached values and get new shipping options
		
	}
	
	// jakal, reset shipping methods for selfservice	
	/*function my_reset_shipping() {
  ...
  function get_shipping_method() {
	...
	//$this->my_reset_shipping();
	  ...
	  // jakal
	  $this->shipping_methods = array('my_shipping'); // \wp-content\plugins\wp-e-commerce\shipping\my-shipping.php
	  $this->shipping_method_count = count($this->shipping_methods);	  
	  
	  $this->selected_shipping_method = $this->shipping_methods[0]; // vybiraem my_shipping kak method
		...
		//if((get_option('do_not_use_shipping') != 1) && (count($this->shipping_methods) > 0)  ) {
			...
			/*if($this->selected_shipping_method != null) {
				...
				//if(is_callable(array(& $wpsc_shipping_modules[$this->selected_shipping_method], "getQuote"  ))) {
					$this->shipping_quotes = $wpsc_shipping_modules[$this->selected_shipping_method]->getQuote($this->calculate_subtotal(), $this->my_shipping_destination['id']);
				//}
			/*} else {
...
//     }
		...
		//if(is_callable(array($wpsc_shipping_modules[$this->selected_shipping_method]), "getQuote"  )) {
			$this->shipping_quotes = $wpsc_shipping_modules[$method]->getQuote($this->calculate_subtotal(), $this->my_shipping_destination['id']);
		//}
...
//exit('hello<pre>'.print_r($new_cart_item,true).'</pre>');
	  ...
	  $status = false;
		...
		$this->clear_cache();
		//do_action("wpsc_cart_updated", &$this); // php 5.3
  ...
  function empty_cart($fromwidget = true) {
		...
		//do_action('wpsc_empty_cart', &$this); // PHP 5.3
  ...
  function calculate_total_shipping() {
	...
	//$this->total_item_shipping = 0;	
	//$total += $this->total_item_shipping;
	
	//$this->total_shipping = $total;
  ...
  function uses_shipping() {
	...
	*/
	function process_as_currency($price) {
		...
		// disable currency sign and filters
		...
		*/
		...
		return $price;  // return only price
  }
  ...
  function get_shipping_quotes() {
	...
	if(is_callable(array($wpsc_shipping_modules[$this->shipping_method], "getQuote"  ))) {
		$unprocessed_shipping_quotes = $wpsc_shipping_modules[$this->shipping_method]->getQuote($this->calculate_subtotal(), $this->my_shipping_destination['id']);
  ...
  // provided values
	...
	*/
	function save_to_db($purchase_log_id) {
		...
		//echo "<pre>";print_r($this,true);echo "</pre>";
		//exit();
		
		// jakal, save delivery date on checkout
		$ddate_str = delivery_date_str_checkout($this->product_id, ($this->cart->selected_shipping_option == "delivery")?true:false);
		$meta_arr = array("deliverydate" => $ddate_str);

		$wpdb->query($wpdb->prepare("INSERT INTO `".WPSC_TABLE_CART_CONTENTS."` (`prodid`, `name`, `purchaseid`, `price`, `pnp`,`tax_charged`, `gst`, `quantity`, `donation`, `no_shipping`, `custom_message`, `files`, `meta`) VALUES ('%d', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%d', '0', '%s', '%s', '%s')", $this->product_id, $this->product_name, $purchase_log_id, $this->unit_price, (float)$shipping, (float)$tax, (float)$tax_rate, $this->quantity, $this->is_donation, $this->custom_message, serialize($this->custom_file), serialize($meta_arr)));
