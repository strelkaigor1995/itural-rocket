...
*/
function wpsc_this_page_url($nopages = false) {
	global $wpsc_query, $is_category_page;
	$output="";
	if($wpsc_query->is_single === true) {
		$output = wpsc_product_url($wpsc_query->product['id']);
	} elseif($wpsc_query->is_search === true) {
		if($is_category_page) { $output = wpsc_category_url($wpsc_query->category); }
		else { $output = get_option('product_list_url'); }
	} elseif($wpsc_query->is_device === true) {
		$output = cs_get_device_url($wpsc_query->query_vars['device_id'], $wpsc_query->query_vars['type']);
	} elseif($wpsc_query->is_cluster === true) {
		$output = cs_get_cluster_url($wpsc_query->query_vars['cluster_id']);
	} else {
		$output = wpsc_category_url($wpsc_query->category);
		// jakal, add flag for gold_shpcrt_view_form, no pages on change number of pages and sort order
		if($nopages == false){
...
/**
* wpsc the product publish function, 
* @return integer - the product publish
*/
function wpsc_the_product_publish() {
	global $wpsc_query;
	return $wpsc_query->product['publish'];
}

function my_is_editor() { // отображение ссылки на ред. товара, дата добавления, посиковый запрос "_by_date_"
...
function wpsc_edit_the_product_link( $link = null, $before = '', $after = '', $id = 0 ) {
	...
	global $wpsc_query, $table_prefix;
...
function wpsc_the_product_description() {
	...
	// disabled tags
	return $wpsc_query->product['description'];
}
...
function wpsc_the_product_price($no_decimals = false) {
	...
	//echo $output."!".$no_decimals."!";
	
	if($no_decimals == true) {
		$output = array_shift(explode(",", $output));
	}
	//else { $output = array_shift(explode(" ", $output)); }
...
function wpsc_product_has_noshipping() {
	global $wpsc_query;
	return $wpsc_query->product['no_shipping'];
}
...
class WPSC_Query {
	...
	// jakal, cats list after search
	var $category_search_arr = array();
	...
	var $is_search = false;
	var $is_search_in_category = false;
	...
	function create_default_query() {
		...
		if(isset($_GET['order'])) {
			...
			$_SESSION['wpsc_product_order'] = $_GET['product_order'];
		...
		if(isset($_GET['items_per_page'])) {
			...
			else if(isset($_SESSION['wpsc_number_per_page'])) {
		...
		// device_id
		if(isset($wp_query->query_vars['device_id']) && ($wp_query->query_vars['device_id'] != '')) {
			$query['device_id'] = $wp_query->query_vars['device_id'];
		} else {
			$query['device_id'] = 0;
		}
		
		// cluster_id
		if(isset($wp_query->query_vars['cluster_id']) && ($wp_query->query_vars['cluster_id'] != '')) {
			$query['cluster_id'] = $wp_query->query_vars['cluster_id'];
		} else {
			$query['cluster_id'] = 0;
		}

		// compat search type: goods service empty (default)
		if(isset($wp_query->query_vars['type']) && ($wp_query->query_vars['type'] != '')) {
			$query['type'] = $wp_query->query_vars['type'];
		} else {
			//$query['type'] = "all";
		}
	...
	**/
	function init_query_flags() {
		...
		$this->is_search_in_category = false;
	...
	**/
	...
	function fill_query_vars($array) {
		...
		//exit('<pre>11'.print_r($array, true).'</pre>');
	...
	function parse_query($query) {
		...
		//exit('<pre>11'.print_r($query, true).'</pre>');
		...
		$qv['product_url_name'] = absint($qv['product_url_name']); // у нас урл это код продукта
		...
		//$qv['featured'] = absint($qv['featured']);
		
		$qv['device_id'] = absint($qv['device_id']);
		$qv['cluster_id'] = absint($qv['cluster_id']);		
		$qv['type'] = trim($qv['type']);
		
		// проверим существует ли страница
		// надо проверять на is_numeric и смотреть активная категория / товар или нет наверно
		/*global $wpdb, $wp_query;
		$is404 = true;
		if(!empty($qv['product_url_name'])){
			if(!empty($qv['category_id'])){
				$result = $wpdb->get_row($wpdb->prepare('SELECT pl.id, ca.category_id FROM wp_wpsc_product_list pl LEFT JOIN wp_wpsc_item_category_assoc ca ON ca.product_id = pl.id WHERE pl.id = %d AND ca.category_id = %d LIMIT 1', $qv['product_url_name'], $qv['category_id']));
				if($wpdb->num_rows > 0) { $is404 = false; }
			}
		} else {
			if(!empty($qv['category_id'])){
				$result = $wpdb->get_row($wpdb->prepare('SELECT * FROM wp_wpsc_product_categories WHERE id = %d', $qv['category_id']));
				if($wpdb->num_rows > 0) { $is404 = false; }
			}
		}
		if(!empty($qv['device_id'])){
			$result = $wpdb->get_var($wpdb->prepare("SELECT id FROM compat_devices WHERE active <> 0 AND id = %d", $qv['device_id']));
			if($wpdb->num_rows > 0) { $is404 = false; }
		}
		if(!empty($qv['cluster_id'])){
			$result = $wpdb->get_var($wpdb->prepare("SELECT id FROM compat_devices WHERE active <> 0 AND id = %d", $qv['device_id']));
			if($wpdb->num_rows > 0) { $is404 = false; }
		}
	...
	function &get_products() {
		...
		//exit('<pre>'.print_r($_GET,true).print_r($_POST,true).print_r($_SESSION,true).print_r($this, true).'</pre>');
		
		//echo "<pre>";debug_print_backtrace();echo "</pre>";
		//echo "<pre>";print_r($this->query_vars);echo "</pre>";
		...
		if($this->query_vars['product_url_name'] != null) {
			...
			$product_id = absint($this->query_vars['product_url_name']);
		...
		if(($product_id > 0)) {
			...
			$product_list = $wpdb->get_results("SELECT * FROM `".WPSC_TABLE_PRODUCT_LIST."` WHERE `id`='".(int)$product_id."' AND `active` IN('1') AND `publish` IN('0','1') LIMIT 1",ARRAY_A);			
		}
		...
		if(count($product_list) > 0) {
			...
			if (isset($_SESSION['item_per_page'])) $products_per_page = $_SESSION['item_per_page'];
			
			//echo $startnum.' / '.$products_per_page;
			
			if ($this->query_vars['featured'] != null) {
			
				// jakal, featured query done here
				$sql = "SELECT p.* FROM wp_wpsc_product_list p LEFT JOIN wp_wpsc_productmeta m ON (m.product_id=p.id) WHERE m.meta_key='featured' and m.meta_value='1' and p.active='1' and p.publish='1'";
			
			} elseif($this->query_vars['top_products_in_category'] != null){
				
				//exit(var_dump($this->query_vars['top_products_in_category']));
				
				$cats_arr = (array)wpsc_list_subcategories($this->query_vars['top_products_in_category']);
				
				//exit(var_dump($cats_arr));
				
				if(empty($cats_arr)){ // нет сабдиректорий, передаем в запрос саму директорию, чтобы не было ошибки sql, а список товаров был пустой
					$cats_arr = array($this->query_vars['top_products_in_category']);
				}
				...
				$sql = "
					...
					FROM (
						SELECT 	pl.*,								
								...
								meta_orders.meta_value orders,
								meta_usluga.meta_value usluga
						FROM wp_wpsc_product_list pl
						LEFT JOIN wp_wpsc_item_category_assoc ca ON pl.id = ca.product_id
						LEFT JOIN wp_wpsc_productmeta meta_orders ON (pl.id = meta_orders.product_id AND meta_orders.meta_key = 'orders')
						LEFT JOIN wp_wpsc_productmeta meta_usluga ON (pl.id = meta_usluga.product_id AND meta_usluga.meta_key = 'usluga')
						LEFT JOIN wp_wpsc_productmeta meta_days ON (pl.id = meta_days.product_id AND meta_days.meta_key = 'deliverydays')
						WHERE CAST(meta_days.meta_value as decimal) >= 0 AND pl.active = '1' AND pl.publish = '1' AND category_id IN (" . implode(', ', $cats_arr) . ")
						ORDER BY CAST(orders AS DECIMAL) DESC
						LIMIT 10
					) foo
					ORDER BY RAND()
				";
				
				//exit('top_products_in_category' . $sql);
			
			} elseif($this->query_vars['analog_products'] != null) {
			
				// jakal, analogs query done here
				
				$an_product_id = $this->query_vars['analog_products'];
				...
				$sql = "
					SELECT 
						...
						meta_orders.meta_value orders,
						meta_usluga.meta_value usluga
					FROM wp_wpsc_product_list p
					LEFT JOIN wp_wpsc_item_category_assoc ca ON ca.product_id = p.id
					LEFT JOIN wp_wpsc_productmeta meta_orders ON (meta_orders.product_id = p.id AND meta_orders.meta_key = 'orders') 
					LEFT JOIN wp_wpsc_productmeta meta_usluga on (p.id = meta_usluga.product_id AND meta_usluga.meta_key = 'usluga') 
					WHERE 
							p.publish='1' 
						AND p.active='1' 
						AND p.id IN (
							SELECT product_id 
							FROM compat_products_clusters 
							WHERE 	
									active=1 
								AND product_id <> {$an_product_id} 
								AND cluster_id IN (
									SELECT cluster_id 
									FROM compat_products_clusters 
									WHERE 	
											product_id = {$an_product_id} 
										AND active=1
								)
						) 
					ORDER BY usluga DESC, CAST(orders as DECIMAL) DESC, name ASC
				";
				
				//exit($sql);
			
			} elseif ($this->query_vars['recommended_products'] != null) {
			
				// jakal, recomnended query done here
				
				$rec_product_id = $this->query_vars['recommended_products'];
				$min_weight = 3;
				$max_products = 10;
				...
				$sql = "
					...
					FROM (
						...
						WHERE 
							...
							AND p.active = '1' 
							AND p.id IN (
								SELECT product_right_id 
								FROM recommended_products 
								WHERE 
										product_left_id = $rec_product_id 
									AND weight >= $min_weight 
								UNION SELECT product_left_id 
								FROM recommended_products 
								WHERE 
										product_right_id = $rec_product_id 
									AND weight >= $min_weight
							) 
							AND p.id NOT IN (
								SELECT product_id 
				...
				//exit('recommended_products' . $sql);
			
			} elseif ($this->query_vars['related_products'] != null) {
				
				// jakal, related query done here, from wpsc_rp_manual product meta
				
				$rp_product_id = $this->query_vars['related_products'];
				
				$related_products = (array)get_product_meta($rp_product_id, 'wpsc_rp_manual', true );
				if(count($related_products) == 1 && $related_products[0] == '0') $related_products = null;
				...
				if($related_products ){
					$sql = "
						...
						WHERE 
								p.publish='1' 
							AND p.active='1' 
							AND p.id<>{$rp_product_id} 
							AND p.id IN (".implode(", ", $related_products).") 
			...
			} elseif(function_exists('gold_shpcrt_search_sql') && ($_GET['product_search'] != '')) {
				
				// search section is done here					
				$this->is_search = true;

				if($_GET['product_search'] == '_by_date_'){
					
					// вся номенклатура по дате добавления для удобства заполнения каталога
					
					if(my_is_editor()) {

						$sql_products_count = "select COUNT(id) from wp_wpsc_product_list where active='1' AND publish='1'";
						$rowcount = $wpdb->get_var($sql_products_count);

						if (isset($_SESSION['item_per_page'])) $products_per_page = $_SESSION['item_per_page'];
						if(!is_numeric($products_per_page) || ($products_per_page < 1)) { $products_per_page = $rowcount; }
						if($startnum >= $rowcount) { $startnum = 0;	}
						
						$sql = "select * from wp_wpsc_product_list where active='1' AND publish='1' order by date_added DESC LIMIT $startnum, $products_per_page";
					...
					$category_search_token11 = "";
					$category_search_token12 = "";
					$category_search_token21 = "";
					$category_search_token22 = "";
					
					//exit(print_r($this));
					
					$category_search_token1 = "";
					$category_search_token2 = "";
					
					if($this->query_vars['category_id'] > 0 ) {
						$this->is_search_in_category = true;
						$this->category_id_list = array_merge((array)$this->category_id_list, (array)wpsc_list_subcategories($this->query_vars['category_id']));
						$category_search_token1 = "AND ca1.category_id IN ('".implode("','", $this->category_id_list)."')";
						$category_search_token2 = "AND ca2.category_id IN ('".implode("','", $this->category_id_list)."')";
					}					
										
					// like tokens search	  
					$search_string_tokens = my_get_search_string();
					
					$sql_likes_products = array();
					$sql_likes_compat = array();
					
					//echo "<pre>";print_r($search_string_tokens);echo "</pre>";
					
					if(count($search_string_tokens) == 1 && strlen($search_string_tokens[0]) == 8 && is_numeric($search_string_tokens[0])){
						// если передали строку из 8 чисел, то ищем и по артикулу
						$search_fields = "CONCAT_WS(' ', p.id, p.name, p.description, p.additional_description) LIKE '%";
					} else {
						$search_fields = "CONCAT_WS(' ', p.name, p.description, p.additional_description) LIKE '%";
					}

					foreach($search_string_tokens as $search_token){
						$sql_likes_products[] = $search_fields . $wpdb->escape($search_token) . "%'";
						//$sql_likes_compat[] = "CONCAT_WS(' ', c.model, c.maker_model, c.name, m.maker, f.family, d.name) LIKE '%".$wpdb->escape($search_token)."%'";
						$sql_likes_compat[] = "CONCAT(c.model, c.maker_model, c.name, m.maker, f.family, d.name) LIKE '%".$wpdb->escape($search_token)."%'";
					}
					
					$sql_condition_products = "AND (".implode(" AND ", $sql_likes_products).")";
					$sql_condition_compat = "AND (".implode(" AND ", $sql_likes_compat).")";
					...
					$sql_products_count = "
						...
						FROM (
							SELECT p.* 
							FROM wp_wpsc_product_list p 
							LEFT JOIN wp_wpsc_item_category_assoc ca1 ON ca1.product_id = p.id
							WHERE 
									p.publish='1' 
								AND p.active='1' 
								$category_search_token1
								$sql_condition_products 
							UNION SELECT p.* 
							FROM wp_wpsc_product_list p 
							LEFT JOIN wp_wpsc_item_category_assoc ca2 ON ca2.product_id = p.id 
							LEFT JOIN compat_products_devices pd ON p.id=pd.product_id 
							LEFT JOIN compat_devices d ON pd.device_id=d.id 
							LEFT JOIN compat_families f ON d.family_id=f.id 
							LEFT JOIN compat_makers m ON f.maker_id=m.id 
							LEFT JOIN compat_products_clusters pc ON p.id=pc.product_id 
							LEFT JOIN compat_clusters c ON pc.cluster_id = c.id 
							WHERE 
									p.publish='1' 
								AND p.active='1' 
								AND pd.active=1 
								...
								AND c.active=1
								$category_search_token2
								$sql_condition_compat
						) foo
					";
					
					//exit($sql_products_count);
					
					$rowcount = $wpdb->get_var($sql_products_count);
					if(!is_numeric($products_per_page) || ($products_per_page < 1)) { $products_per_page = $rowcount; }
					if($startnum >= $rowcount) { $startnum = 0; }
					
					$order_by_last = "name ASC";
					$default_order_by = "usluga DESC, CAST(orders as DECIMAL) DESC, $order_by_last";
					
					if($this->query_vars['sort_order']=='NAME'){
						$order_by = $order_by_last;
					} elseif ($this->query_vars['sort_order']=='ASC') {
						$order_by = "price ASC, $order_by_last";
					} elseif ($this->query_vars['sort_order']=='DESC') {
						$order_by = "price DESC, $order_by_last";
					} elseif ($this->query_vars['sort_order']=='ORDERS') {
						$order_by = $default_order_by;
					} elseif ($this->query_vars['sort_order']=='DAYS') {
						$order_by = "usluga DESC, deliverydays ASC, $order_by_last";
					} elseif ($this->query_vars['sort_order']=='DATE') {
						$order_by = "date_added DESC, $order_by_last";
					} else {
						$order_by = $default_order_by;
					}
					
					//$sql = "SELECT p.* FROM wp_wpsc_product_list p LEFT JOIN compat_products_devices pd ON p.id=pd.product_id LEFT JOIN compat_devices d ON pd.device_id=d.id LEFT JOIN compat_families f ON d.family_id=f.id LEFT JOIN compat_makers m ON f.maker_id=m.id LEFT JOIN compat_products_clusters pc ON p.id=pc.product_id LEFT JOIN compat_clusters c ON pc.cluster_id = c.id WHERE p.publish='1' AND p.active='1' AND pd.active=1 AND pc.active=1 $sql_condition GROUP BY p.id ORDER BY $order_by LIMIT $startnum, $products_per_page";
					...
					$sql = "
						...
						FROM (
							SELECT 
								...
								meta_orders.meta_value orders, 
								IF(CAST(meta_ddays.meta_value AS DECIMAL) = -1, 999999, IF(CAST(meta_ddays.meta_value AS DECIMAL) = -2, 888888, CAST(meta_ddays.meta_value AS DECIMAL))) deliverydays,
								meta_usluga.meta_value usluga
							FROM wp_wpsc_product_list p 
							LEFT JOIN wp_wpsc_productmeta meta_orders ON (meta_orders.product_id = p.id AND meta_orders.meta_key = 'orders') 
							LEFT JOIN wp_wpsc_productmeta meta_usluga on (p.id = meta_usluga.product_id AND meta_usluga.meta_key = 'usluga') 
							LEFT JOIN wp_wpsc_productmeta meta_ddays ON (meta_ddays.product_id = p.id AND meta_ddays.meta_key = 'deliverydays') 
							LEFT JOIN wp_wpsc_item_category_assoc ca1 ON ca1.product_id = p.id
							WHERE 
									p.publish='1' 
								AND p.active='1' 
					...
					$sql_cats = "
						...
						FROM (
							...
							FROM (
								...
								FROM wp_wpsc_product_list p 
								LEFT JOIN wp_wpsc_item_category_assoc ca1 ON ca1.product_id = p.id 
								LEFT JOIN wp_wpsc_product_categories pc1 ON pc1.id = ca1.category_id 
								WHERE 
										p.publish='1' 
									AND p.active='1' 
									$category_search_token1
								...
								FROM wp_wpsc_product_list p 
								LEFT JOIN wp_wpsc_item_category_assoc ca2 ON ca2.product_id = p.id 
								LEFT JOIN wp_wpsc_product_categories pc2 ON pc2.id = ca2.category_id 
								LEFT JOIN compat_products_devices pd ON p.id=pd.product_id 
								LEFT JOIN compat_devices d ON pd.device_id=d.id 
								LEFT JOIN compat_families f ON d.family_id=f.id 
								LEFT JOIN compat_makers m ON f.maker_id=m.id 
					...
					$this->category_search_arr = $wpdb->get_results($sql_cats,ARRAY_A);
					
					if(count($this->category_search_arr) == 1){
						$this->query_vars['category_id'] = $this->category_search_arr[0]['category_id'];
						/*if(!isset($_GET['redirected'])){					
							$url = add_query_arg('product_search', $_GET['product_search'], wpsc_category_url($this->category_search_arr[0]['category_id']));
			...
			*/
			...
			} elseif($this->query_vars['device_id'] > 0) {
				
				// compatible search done here
				$this->is_device = true;

				$device_id = $this->query_vars['device_id'];
				
				// get device name for use in theme
				$this->device_name = cs_get_device_name($device_id);
				
				//exit($device_id);
				
				switch($this->query_vars['type']){
					case 'service':
						$meta_value = "='1'";
						break;
					case 'goods':
				...
				$rowcount = $wpdb->get_var("
					SELECT 
						COUNT(p.id) 
					FROM wp_wpsc_product_list p 
					LEFT JOIN wp_wpsc_productmeta m on p.id = m.product_id
					WHERE 
							m.meta_key = 'usluga' 
						AND m.meta_value{$meta_value} 
						AND p.publish = '1' 
						AND p.active = '1' 
						AND p.id IN (SELECT pd.product_id FROM compat_products_devices pd LEFT JOIN compat_devices d ON pd.device_id=d.id WHERE pd.device_id = {$device_id} AND pd.active = 1 AND d.active=1)
				");
				
				// запишем историю запросов в БД
				compat_search_log($device_id, $this->query_vars['type'], $rowcount);

				if (isset($_SESSION['item_per_page'])) $products_per_page = $_SESSION['item_per_page'];
				if(!is_numeric($products_per_page) || ($products_per_page < 1)) { $products_per_page = $rowcount; }
				if($startnum >= $rowcount) { $startnum = 0;	}
				
				$order_by_last = "p.name ASC";
				$default_order_by = "usluga DESC, CAST(orders as DECIMAL) DESC, $order_by_last";
				
				if($this->query_vars['sort_order']=='NAME'){
					$order_by = $order_by_last;
				} elseif ($this->query_vars['sort_order']=='ASC') {
					$order_by = "p.price ASC, $order_by_last";
				} elseif ($this->query_vars['sort_order']=='DESC') {
					$order_by = "p.price DESC, $order_by_last";
				} elseif ($this->query_vars['sort_order']=='ORDERS') {
					$order_by = $default_order_by;
				} elseif ($this->query_vars['sort_order']=='DAYS') {
					$order_by = "usluga DESC, deliverydays ASC, $order_by_last";
				...
				$sql = "
					SELECT 
						...
						meta_usluga.meta_value usluga
						FROM wp_wpsc_product_list p 
						LEFT JOIN wp_wpsc_item_category_assoc ca ON p.id = ca.product_id
						LEFT JOIN wp_wpsc_productmeta meta_usluga on (p.id = meta_usluga.product_id AND meta_usluga.meta_key = 'usluga') 
						LEFT JOIN wp_wpsc_productmeta meta_orders ON (meta_orders.product_id = p.id AND meta_orders.meta_key = 'orders') 
						LEFT JOIN wp_wpsc_productmeta meta_ddays ON (meta_ddays.product_id = p.id AND meta_ddays.meta_key = 'deliverydays') 
						WHERE 
								meta_usluga.meta_value{$meta_value} 
							AND p.publish = '1' 
							AND p.active = '1' 
							AND p.id IN (SELECT pd.product_id FROM compat_products_devices pd LEFT JOIN compat_devices d ON pd.device_id=d.id WHERE pd.device_id = {$device_id} AND pd.active = 1 AND d.active=1) 
							ORDER BY $order_by 
			...
			} elseif($this->query_vars['cluster_id'] > 0) { 
			
				// model search 
				$this->is_cluster = true;
				
				$cluster_id = $this->query_vars['cluster_id'];
				
				// get cluster name for use in theme
				$this->cluster_name = cs_get_cluster_name($cluster_id);
				...
				$rowcount = $wpdb->get_var("
					...
					WHERE 
						...
						AND p.publish = '1' 
						AND p.active = '1' 
						AND p.id IN (SELECT pc.product_id FROM compat_products_clusters pc LEFT JOIN compat_clusters c ON pc.cluster_id=c.id WHERE pc.cluster_id = {$cluster_id} AND pc.active = 1 AND c.active=1)
				");
				...
				if(is_numeric($this->query_vars['category_id']) && ($this->query_vars['category_id'] > 0)) {
					...
					// выводим товары из вложенных категорий
					//$this->category_id_list = array_merge((array)$this->category_id_list, (array)wpsc_list_subcategories($this->query_vars['category_id']));
					...
					$order_by_last = "p.name ASC";
					$default_order_by = "usluga DESC, CAST(col_orders as DECIMAL) DESC, $order_by_last";
					...
					// jakal, no manual sort
					$rowcount_sql = "
						SELECT 
							COUNT(DISTINCT p.id) 
						FROM wp_wpsc_product_list p 
						LEFT JOIN wp_wpsc_item_category_assoc ca ON p.id = ca.product_id 
						LEFT JOIN wp_wpsc_product_categories c ON ca.category_id = c.id 
						WHERE 
								p.publish = '1' 
							AND p.active = '1' 
							AND ca.category_id IN ('".implode("','", $this->category_id_list)."')
					";
					
					$rowcount = $wpdb->get_var($rowcount_sql);
					...
					$sql = "
						SELECT
							...
							pc.name category, 
							ca.category_id, 
							meta_orders.meta_value col_orders, 
							IF(CAST(meta_ddays.meta_value AS DECIMAL) = -1, 999999, IF(CAST(meta_ddays.meta_value AS DECIMAL) = -2, 888888, CAST(meta_ddays.meta_value AS DECIMAL))) deliverydays,
							meta_usluga.meta_value usluga
						FROM wp_wpsc_product_list p 
						LEFT JOIN wp_wpsc_item_category_assoc ca ON p.id = ca.product_id 
						LEFT JOIN wp_wpsc_product_categories pc ON ca.category_id = pc.id 
						LEFT JOIN wp_wpsc_productmeta meta_orders ON (meta_orders.product_id = p.id AND meta_orders.meta_key = 'orders') 
						LEFT JOIN wp_wpsc_productmeta meta_ddays ON (meta_ddays.product_id = p.id AND meta_ddays.meta_key = 'deliverydays') 
						LEFT JOIN wp_wpsc_productmeta meta_usluga ON (meta_usluga.product_id = p.id AND meta_usluga.meta_key = 'usluga')
						WHERE 
								p.publish = '1' 
							AND p.active = '1' 
							AND ca.category_id IN ('".implode("','", $this->category_id_list)."') 
					...
					// jakal, disable products on products home
					...
					// jakal, no sort
					$rowcount = $wpdb->get_var("SELECT COUNT(id) FROM `wp_wpsc_product_list` WHERE `publish`='1' AND `active`='1'");
					
					if(!is_numeric($products_per_page) || ($products_per_page < 1)) { $products_per_page = $rowcount; }
					if($startnum >= $rowcount) { $startnum = 0;	}
					
					//$sql = "SELECT DISTINCT `".WPSC_TABLE_PRODUCT_LIST."`.*, `".WPSC_TABLE_PRODUCT_ORDER."`.`order` FROM `".WPSC_TABLE_PRODUCT_LIST."` LEFT JOIN `".WPSC_TABLE_ITEM_CATEGORY_ASSOC."` ON `".WPSC_TABLE_PRODUCT_LIST."`.`id` = `".WPSC_TABLE_ITEM_CATEGORY_ASSOC."`.`product_id` LEFT JOIN `".WPSC_TABLE_PRODUCT_ORDER."` ON `".WPSC_TABLE_PRODUCT_LIST."`.`id` = `".WPSC_TABLE_PRODUCT_ORDER."`.`product_id` WHERE `".WPSC_TABLE_PRODUCT_LIST."`.`publish`='1' AND `".WPSC_TABLE_PRODUCT_LIST."`.`active`='1'  $no_donations_sql $group_sql ORDER BY `".WPSC_TABLE_PRODUCT_LIST."`.`special`, $order_by LIMIT $startnum, $products_per_page";
					 
					// jakal, no order, no sort on special
					//$sql = "SELECT DISTINCT `".WPSC_TABLE_PRODUCT_LIST."`.*, 0 as `order` FROM `".WPSC_TABLE_PRODUCT_LIST."` LEFT JOIN `".WPSC_TABLE_ITEM_CATEGORY_ASSOC."` ON `".WPSC_TABLE_PRODUCT_LIST."`.`id` = `".WPSC_TABLE_ITEM_CATEGORY_ASSOC."`.`product_id` WHERE `".WPSC_TABLE_PRODUCT_LIST."`.`publish`='1' AND `".WPSC_TABLE_PRODUCT_LIST."`.`active`='1'  $no_donations_sql $group_sql ORDER BY $order_by LIMIT $startnum, $products_per_page";
					 
					// jakal, and no distinct
					$sql = "SELECT * FROM `wp_wpsc_product_list` WHERE `publish`='1' AND `active`='1' $group_sql ORDER BY $order_by LIMIT $startnum, $products_per_page";
