...
//   exit('<pre>'.print_r($product,true).'</pre>');
  if($state == true) {
		$cart_messages[] = str_replace("[product_name]", stripslashes($product['name']), 'Вы добавили "[product_name]" в корзину');
	} else {
...
/*if($_REQUEST['wpsc_ajax_action'] == 'update_shipping_price') {
	add_action('init', 'my_update_shipping');
}*/
...
function wpsc_submit_checkout() {
	...
	// jakal, if selfservice checked, then set no delivery
	/*$checkout_fields = $_POST['collected_data'];
	if($checkout_fields[23] != ""){
		my_update_shipping(false);
	}*/
	...
	if($is_valid == true || $_GET['gateway'] == 'noca') {
		...
		$purchase_log_id = my_purchase_log_save_to_db($wpsc_cart, $sessionid, $submitted_gateway);
		
		$wpsc_checkout->save_forms_to_db($purchase_log_id, $_POST['collected_data']);
...
function compat_get_makers(){

	global $wpdb;

	// такой же запрос в \wp-content\api\v1\getProducts.php

	//$results = $wpdb->get_results("SELECT DISTINCT m.id, m.maker FROM compat_products_devices p LEFT JOIN compat_devices d ON p.device_id=d.id LEFT JOIN compat_families f ON d.family_id=f.id LEFT JOIN compat_makers m ON f.maker_id=m.id WHERE m.active=1 AND p.active=1 ORDER BY m.maker", ARRAY_A);
	$results = $wpdb->get_results("SELECT m.id, m.maker FROM compat_makers m WHERE active=1 ORDER BY m.maker", ARRAY_A);

	$data = array();

	foreach($results as $row){
		$data[] = array('id' => $row['id'], 'maker' => $row['maker']);
	}
...
if($_REQUEST['wpsc_ajax_action'] == 'compat_get_makers') {
	add_action('init', 'compat_get_makers');
}

function compat_get_families(){
	...
	if(isset($_GET['maker_id'])){
		...
		$maker_id = preg_replace('/[^0-9]/', '', $_GET['maker_id']);
		
		if($maker_id != ''){
			
			// такой же запрос в \wp-content\api\v1\getProducts.php
			
			//$results = $wpdb->get_results("SELECT DISTINCT f.id, f.family FROM compat_products_devices p LEFT JOIN compat_devices d ON p.device_id=d.id LEFT JOIN compat_families f ON d.family_id=f.id LEFT JOIN compat_makers m ON f.maker_id=m.id LEFT JOIN wp_wpsc_product_list pl ON pl.id=p.product_id WHERE pl.publish='1' AND pl.active='1' AND p.active=1 and m.id=$maker_id AND f.active=1 ORDER BY f.family", ARRAY_A);
			$results = $wpdb->get_results("SELECT f.id, f.family FROM compat_families f LEFT JOIN compat_makers m ON f.maker_id=m.id WHERE m.id=$maker_id AND f.active=1 ORDER BY f.family", ARRAY_A);
			
			foreach($results as $row){
				$data[] = array('id' => $row['id'], 'family' => $row['family']);
			}
		}
...
if($_REQUEST['wpsc_ajax_action'] == 'compat_get_families') {
	add_action('init', 'compat_get_families');
}

function compat_get_devices(){
	...
	if(isset($_GET['family_id'])){
		...
		$family_id = preg_replace('/[^0-9]/', '', $_GET['family_id']);
		
		if($family_id != ''){
			
			// такой же запрос в \wp-content\api\v1\getProducts.php
			
			//$results = $wpdb->get_results("SELECT DISTINCT d.id, d.name, CAST(d.name as UNSIGNED) as c, IF(LEFT(d.name, 1) > 0, 0, 1) as isfl, LEFT(d.name, 1) as fl, LENGTH(d.name) as l FROM compat_products_devices p LEFT JOIN compat_devices d ON p.device_id=d.id LEFT JOIN compat_families f ON d.family_id=f.id LEFT JOIN wp_wpsc_product_list pl ON pl.id=p.product_id WHERE pl.publish='1' AND pl.active='1' AND p.active=1 AND f.id=$family_id AND d.active=1 ORDER BY isfl ASC, c ASC, fl ASC, l ASC, d.name ASC", ARRAY_A);
			$results = $wpdb->get_results("SELECT d.id, d.name, CAST(d.name AS UNSIGNED) AS c, IF(LEFT(d.name, 1) > 0, 0, 1) AS isfl, LEFT(d.name, 1) AS fl,LENGTH(d.name) AS l FROM compat_devices d LEFT JOIN compat_families f ON d.family_id=f.id WHERE f.id=$family_id AND d.active=1 ORDER BY isfl ASC, c ASC, fl ASC, l ASC, d.name ASC", ARRAY_A);
			
			foreach($results as $row){
				$data[] = array('id' => $row['id'], 'device' => $row['name']);
			}
		}
...
if($_REQUEST['wpsc_ajax_action'] == 'compat_get_devices') {
	add_action('init', 'compat_get_devices');
}

function compat_model_search(){
	...
	if(isset($_GET['query'])){
		...
		if($query != ''){
			...
			$sql = "SELECT id, model, maker_model, name FROM compat_clusters WHERE active=1";
			
			$search_string_tokens = preg_split("/[\/\s-(),.]/", $query, NULL, PREG_SPLIT_NO_EMPTY);
			foreach($search_string_tokens as $search_token){
				$concat_likes[] = "CONCAT_WS(model, maker_model, name) LIKE '%".$wpdb->escape($search_token)."%'";
			}
			$sql .= " AND ".implode(" AND ", $concat_likes);
			...
			foreach($results as $row){
				$data[] = array('id' => $row['id'], 'model' => $row['model'], 'maker_model' => $row['maker_model'], 'name' => $row['name']);
			}
		}
...
if($_REQUEST['wpsc_ajax_action'] == 'compat_model_search') {
	add_action('init', 'compat_model_search');
}

function compat_get_count(){
	...
	if(isset($_GET['device_id'])){
		...
		$device_id = preg_replace('/[^0-9]/', '', $_GET['device_id']);
		
		if($device_id != ''){
			if(isset($_GET['type'])){
				if($_GET['type'] == 'service') { $meta_value = 1; } else { $meta_value=0; }
				$rowcount = $wpdb->get_var("SELECT COUNT(p.id) FROM wp_wpsc_product_list p LEFT JOIN wp_wpsc_productmeta m on (p.id=m.product_id) WHERE m.meta_key='usluga' AND m.meta_value='{$meta_value}' AND p.publish='1' AND p.active='1' AND p.id IN (SELECT product_id FROM compat_products_devices WHERE device_id={$device_id} AND active=1)");
			} else {
				$rowcount = $wpdb->get_var("SELECT COUNT(p.id) FROM wp_wpsc_product_list p WHERE p.publish='1' AND p.active='1' AND p.id IN (SELECT product_id FROM compat_products_devices WHERE device_id={$device_id} AND active=1)");
			}
		}
...
if($_REQUEST['wpsc_ajax_action'] == 'compat_get_count') {
	add_action('init', 'compat_get_count');
}

function my_get_cart_class_ajax(){
	
	header('Content-type: application/json');
	echo json_encode(array('cart_class' => my_get_cart_class(), 'link_text' => my_get_cart_link_text()));

	exit();
	
}

if($_REQUEST['wpsc_ajax_action'] == 'get_cart_class') {
	add_action('init', 'my_get_cart_class_ajax');
}

function my_shipping_api(){
	...
	if(isset($_GET['action'])){
		...
		switch($_GET['action']){
			/*case 'getDestinations':
				$result = array();
				$results = $wpdb->get_results("select id, name from shipping_destinations order by `order` desc", ARRAY_A);
				foreach($results as $row){	
					$selected = ($wpsc_cart->my_shipping_destination['id'] == $row['id']) ? 1 : 0;
					$result[] = array('id' => $row['id'], 'name' => $row['name'], 'selected' => $selected);
				}			
				break;*/
			/*case 'getCartTotal':
				$result = wpsc_cart_total(true);
				break;*/
			case 'setShippingOptionPickup':
				$wpsc_cart->update_shipping($wpsc_cart->selected_shipping_method, 'pickup');
				$result['CartTotal'] = wpsc_cart_total(true);
				$result['ShippingOption'] = $wpsc_cart->selected_shipping_option;
				break;
			case 'setShippingOptionDelivery':
				$wpsc_cart->update_shipping($wpsc_cart->selected_shipping_method, 'delivery');
				$result['CartTotal'] = wpsc_cart_total(true);
				$result['ShippingOption'] = $wpsc_cart->selected_shipping_option;
				break;
			/*case 'getShippingOption':
				$result = $wpsc_cart->selected_shipping_option;
				break;*/
			/*case 'getCart':
				$result = array('CartTotal' => wpsc_cart_total(true), 'ShippingOption' => $wpsc_cart->selected_shipping_option);
				break;*/
		}
...
if($_REQUEST['wpsc_ajax_action'] == 'shipping_api') {
	add_action('init', 'my_shipping_api');
}
