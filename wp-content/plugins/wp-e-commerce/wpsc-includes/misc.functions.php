  ...
  if(stristr($post->post_content,'[productspage]')) {
		...
		if($wpsc_query->is_single){
                ...
                //if(!is_numeric($_GET['product_id'])) {
					...
					$product_id = absint($wpsc_query->query_vars['product_url_name']);
		...
		if ($wpsc_query->is_category) {
			...
			$url = add_query_arg('items_per_page', 'all', $url);
		...
		if($wpsc_query->is_device){
			$url = cs_get_device_url($wpsc_query->query_vars['device_id'], $wpsc_query->query_vars['type']);
			$url = add_query_arg('items_per_page', 'all', $url);
		}

		if($wpsc_query->is_cluster){
			$url = cs_get_cluster_url($wpsc_query->query_vars['cluster_id']);
			$url = add_query_arg('items_per_page', 'all', $url);
		}
...
function wpsc_change_next_link($url){
	...
	if(wpsc_has_pages()){
		...
		if($current_page < $last){
			...
			if($wpsc_query->is_category){
				return wpsc_category_url(absint($wpsc_query->query_vars['category_id'])) . 'page/' . ($current_page + 1);
			}
			
			if($wpsc_query->is_device){
				return cs_get_device_url($wpsc_query->query_vars['device_id'], $wpsc_query->query_vars['type'], $current_page + 1);
			}
			
			if($wpsc_query->is_cluster){
				return cs_get_cluster_url($wpsc_query->query_vars['cluster_id'], $current_page + 1);
			}
...
add_filter('aioseop_next_link', 'wpsc_change_next_link');

function wpsc_change_prev_link($url){
...
add_filter('aioseop_prev_link', 'wpsc_change_prev_link');
...
function my_wpsc_change_page_title($title) {
	$new_title = wpsc_obtain_the_title();
	if($new_title != '') {
	  $title = $new_title;
	}
...
add_filter('aioseop_title_page', 'my_wpsc_change_page_title');
...
function wpsc_set_aioseop_description($data) {
	...
	if($wpsc_query->is_single) {
		...
		// у нас урл продукта = коду продукта
		$product_id = absint($wp_query->query_vars['product_url_name']);
		
		$usluga = get_product_meta($product_id, 'usluga');
		$verified = get_product_meta($product_id, 'verified');
		$vendor = get_product_meta($product_id, 'vendor');
		//$typePrefix = get_product_meta($product_id, 'typePrefix');
		
		$cluster_arr = $wpdb->get_results(
			$wpdb->prepare('
				SELECT cc.id, cc.model, cc.maker_model, cc.name, cm.maker, ct.name `type`
				FROM compat_products_clusters cpc
				LEFT JOIN compat_clusters cc ON cc.id = cpc.cluster_id
				LEFT JOIN compat_makers cm ON cm.id = cc.maker_id
				LEFT JOIN compat_types ct ON ct.id = cc.type_id
				WHERE cpc.product_id = %d AND cpc.active = 1 LIMIT 1', 
				$product_id
			), 
			ARRAY_A
		);		
		...
		foreach($cluster_arr as $cluster){
			...
			if(empty($usluga)){
				$new_data = cs_get_cluster_name($cluster['id']) . ($vendor == $cluster['maker']?'':' ' . $vendor) . ' купить в Екатеринбурге по низкой цене в интернет-магазине ИТ Сервис с доставкой или самовывозом. Смотрите список совместимых принтеров на нашем сайте.';
			} else {
				$new_data = 'Заправка картриджа для принтера ' . cs_get_cluster_name($cluster['id']) . ' в Екатеринбурге по низкой цене в компании ИТ Сервис. Выезд курьера.' . (empty($verified)?'':' Качество заправки проверяется при вас.');
			}
	...
	if($wpsc_query->is_home){
		$new_data = 'Интернет магазин расходных материалов для оргтехники и сервис по заправке картриджей в Екатеринбурге. Мы предлагаем большой ассортимент картриджей, тонера, чернил, СНПЧ, ПЗК и запасных частей для оргтехники.';
	}

	if($wpsc_query->is_category){		
		$new_data = ($wpsc_query->breadcrumb_count > 2?$wpsc_query->breadcrumbs[$wpsc_query->breadcrumb_count - 2]['name'] . ' ':'') . wpsc_category_name($wpsc_query->category) . ' по низким ценам в Екатеринбурге в компании ИТ Сервис. Мы предлагаем товары с доставкой и самовывозом.';
	}
	...
	if($wpsc_query->is_device){
		...
		switch($wpsc_query->query_vars['type']){
			case 'goods':
				...
				break;
			case 'service':
				$prefix = 'Заправка картриджей';
				break;
			default:
				$prefix = 'Картриджи и заправка картриджей';
		}
		
		$new_data = $prefix . ' для принтера ' . $wpsc_query->device_name . '. Большой выбор картриджей для принтеров ' . cs_get_maker_name($wpsc_query->query_vars['device_id']) . ' по низким ценам c доставкой и самовывозом.';
...
function wpsc_set_aioseop_keywords($data) {
	...
	if($wpsc_query->is_single) {
		...
		$cluster_arr = $wpdb->get_results(
			$wpdb->prepare('
				SELECT cc.model, cc.maker_model, cc.name, cm.maker, ct.name `type`
				FROM compat_products_clusters cpc
				LEFT JOIN compat_clusters cc ON cc.id = cpc.cluster_id
				LEFT JOIN compat_makers cm ON cm.id = cc.maker_id
				LEFT JOIN compat_types ct ON ct.id = cc.type_id
				WHERE cpc.product_id = %d AND cpc.active = 1', 
				$product_id
			), 
			ARRAY_A
		);
		...
		foreach($cluster_arr as $cluster){
			...
			$keywords[] = $cluster['maker'] . ' ' . $cluster['model'];
			$keywords[] = $cluster['maker'] . ' ' . $cluster['maker_model'];
			
			if(!empty($vendor)){
				$keywords[] = $vendor . ' ' . $cluster['model'];
				$keywords[] = $vendor . ' ' . $cluster['maker_model'];				
			}
			
			$keywords[] = $cluster['model'];
			$keywords[] = $cluster['maker_model'];
			
			if(!empty($usluga)){
				$keywords[] = 'Заправка картриджа';
			}

			//$keywords[] = $cluster['name'];
			//$keywords[] = $cluster['maker'];
			
			$keywords[] = $cluster['type'] . (empty($typePrefix)?'':' '.$typePrefix);			
			
		}

		$new_data = mb_strtolower(implode(', ', array_unique($keywords)));
		
	}
	
	if($wpsc_query->is_home){
		$new_data = 'интернет-магазин, картриджи, Екатеринбург';
	}
	...
	if($wpsc_query->is_device){		
		...
		$new_data = $prefix . ' ' . $wpsc_query->device_name . ', принтер ' . $wpsc_query->device_name;
	}
...
function nzshpcrt_display_preview_image() {
	  ...
	  if(($_GET['wpsc_request_image'] == 'true') || is_numeric($_GET['productid']) || is_numeric($_GET['image_id'])|| isset($_GET['image_name'])) {
		...
		if(function_exists("getimagesize")) {
			...
			if($use_cache === true ) {
				//echo "Location: ".$cache_url.$cache_filename.$extension;
...
function my_get_destinations($destination_id = null){

	global $wpdb;
	
	if($destination_id !== null){
		$result_arr = $wpdb->get_row($wpdb->prepare("SELECT id, name, `order`, fias_type, fias_id FROM shipping_destinations WHERE id = %d LIMIT 1", $destination_id), ARRAY_A);
	} else {
		$result_arr = $wpdb->get_results("SELECT id, name, `order`, fias_type, fias_id FROM shipping_destinations order by `order` desc", ARRAY_A);
	}
...
function my_get_location_by_ip($ip){
	...
	if(!empty($ip)){
		...
		$sql = $wpdb->prepare("SELECT dadata FROM dadata_log WHERE ip_n = INET6_ATON(%s) AND DATE(`datetime`) > DATE_SUB(CURDATE(), INTERVAL 1 DAY)", $ip);
		$result = $wpdb->get_var($sql);
		
		//exit('result' . var_dump($result));
		
		if($wpdb->num_rows === 0){

			$service_url = 'https://suggestions.dadata.ru/suggestions/api/4_1/rs/detectAddressByIp?ip=' . $ip;
			
			/*$curl = curl_init($service_url);
			
			$headers = array( 
				"Accept: application/json", 
				"Authorization: Token 06d110ec5168bc840a8a62f26689f91a73fe7292"
			);
			...
			if ($curl_response !== false) {
				$location = json_decode($curl_response);
			} else {
				$location = false;
         }*/
			
			//exit('$wpdb->num_rows' . var_dump($location));

			$wpdb->query(
				$wpdb->prepare(
					"INSERT INTO dadata_log (
						datetime,
						ip_n,
						ip, 
						dadata, 
