...
// используем в wpsc_submit_checkout() и \wp-content\api\v1\sendOrder.php
function my_purchase_log_save_to_db(&$cart, $sessionid, $submitted_gateway){
	
	global $wpdb, $user_ID, $wpsc_shipping_modules;
	
	$base_shipping = $cart->calculate_base_shipping();
	$total = $cart->calculate_subtotal();
	$total += $cart->calculate_total_shipping();
	
	$find_us = '';

	if($cart->selected_shipping_option == 'pickup'){ // самовывоз
		$shipping_product_id = 'NULL';
	} else {
		$shipping_product_id = isset($wpsc_shipping_modules[$cart->selected_shipping_method]->product_id) ? $wpsc_shipping_modules[$cart->selected_shipping_method]->product_id : 'NULL';
	}
	
	// shipping_product_id - не умеет вставлять NULL
	$sql = $wpdb->prepare(
		'INSERT INTO `wp_wpsc_purchase_logs` (`totalprice`,`statusno`, `sessionid`, `user_ID`, `date`, `gateway`, `billing_country`,`shipping_country`, `billing_region`, `shipping_region`, `base_shipping`,`shipping_method`, `shipping_option`, `plugin_version`, `discount_value`, `discount_data`,`find_us`,`shipping_destination_id`,`shipping_product_id`) VALUES (%f, %d, %s, %d, UNIX_TIMESTAMP(), %s, %s, %s, %s, %s, %f, %s, %s, %s, %f, %s, %s, %d, ' . $shipping_product_id . ')',
		$total,
		0,
		$sessionid,
		(int)$user_ID,
		$submitted_gateway,
		$cart->selected_country, 
		$cart->delivery_country,
		$cart->selected_region,
		$cart->delivery_region,
		$base_shipping,
		$cart->selected_shipping_method,
		$cart->selected_shipping_option,
		WPSC_VERSION,
		$cart->coupons_amount,
		$cart->coupons_name,
		$find_us,
		$cart->my_shipping_destination['id']
	);
...
class wpsc_purchaselogs{
	...
	function getdates(){
		...
		if(!isset($purchlogs)) $purchlogs = new stdClass();
...
*/
	function the_purch_item_name(){
		...
		//echo '<pre>';print_r($this->form_data);echo '</pre>';
		//exit();
		...
		// jakal, get email, fname and lname from formdata
		$emailformid = 8;
		$fNameformid = 2;
		//$lNameformid = ;
		...
		$namestring = $fname.' (<a href="mailto:'.$email.'?subject=Message From '.get_option('siteurl').'">'.$email.'</a>) ';
