...
function wpsc_display_products_page($query) {
  global $wpdb, $wpsc_query, $wpsc_theme_path, $is_search_page;
  ...
  //exit("wpsc_display_products_page");
	...
	if(wpsc_is_single_product()) {
		...
		include($cur_wpsc_theme_folder."/search_page.php");
	} else {
		/*
...
function wpsc_products_page($content = '') {
	...
	global $wpdb, $wp_query, $wpsc_query, $wpsc_theme_path, $is_search_page;
	
	/// added by xiligroup.dev to be compatible with touchshop
	$cur_wpsc_theme_folder = apply_filters('wpsc_theme_folder',$wpsc_theme_path.WPSC_THEME_DIR);
	/// end of added by xiligroup.dev to be compatible with touchshop
  
	//exit("wpsc_products_page");
	...
	if(preg_match("/\[productspage\]/", $content)) {
			...
			if(wpsc_is_single_product()) {
				...
				include($cur_wpsc_theme_folder."/search_page.php");
			} elseif($wpsc_query->is_category) {
				include($cur_wpsc_theme_folder."/category_page.php");
			} else {
			    
				/* jakal, disable display type per category, always default
				...
				// jakal, products_page.php is only one what we needed
				include($cur_wpsc_theme_folder."/products_page.php");
