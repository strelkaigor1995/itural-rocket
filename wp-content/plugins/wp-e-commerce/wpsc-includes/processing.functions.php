...
function nzshpcrt_currency_display($price_in, $tax_status = 0, $nohtml = false, $id = false, $no_dollar_sign = false, $decimals = 2) {
  ...
  function nzshpcrt_determine_base_shipping($per_item_shipping, $country_code) {    
    ...
    if((get_option('do_not_use_shipping') != 1) && (count($custom_shipping) > 0)) {
			...
			if($_SESSION['quote_shipping_method'] != null) {
			  ...
			  $shipping_quotes = $wpsc_shipping_modules[$_SESSION['quote_shipping_method']]->getQuote($wpsc_cart->calculate_subtotal(), $wpsc_cart->my_shipping_destination['id']);
			} else {
