...
function TB_show(caption, url, rel) {
	...
	if ( baseURL.match(imageURL) ) {
		...
		if ( rel ) {
			...
			var foundSelf = false;
			var imageTitle = 'Галерея'; //default to something sane
			...
			for (var i = 0; i < imageGroup.length; i++) {
				...
				if ( image.href == url ) {
					foundSelf = true;
					imageCount = "Изображение " + (i + 1) + " из&nbsp;"+ (imageGroup.length)+".";
          if(image.rel != null) {
            //imageTitle = image.rel.replace(/_/, " ");
					...
					if ( foundSelf ) {
						next = getInfo(image, "next", "След. &gt;");
						// stop searching
						break;
					} else {
						// didn't find ourself yet, so this may be the one before ourself
						prev = getInfo(image, "prev", "&lt; Пред.");
					}
				}
			}
		}
      ...
      if(imageGroup.length > 1) { 
       ...
       jQuery("#TB_window").append("<div id='TB_Header'><div id='TB_TopCloseAjaxWindow'><a href='#' id='TB_TopCloseWindowButton' title='Закрыть галерею'>Закрыть</a></div>"+imageTitle+"</div>");
        ...
        jQuery("#TB_TopCloseWindowButton").click(TB_remove);
          ...
          TB_HEIGHT = imageHeight + 60;
			...
			jQuery("#TB_window").append("<a href='' id='TB_ImageOff' title='Кликните, чтобы закрыть галерею'><img id='TB_Image' src='"+url+"' width='"+imageWidth+"' height='"+imageHeight+"' alt='"+caption+"'/></a>" + "<div id='TB_caption'>"+caption+"<div id='TB_secondLine'>" + imageCount + prev.html + next.html + "</div></div><div id='TB_closeWindow'><a href='#' id='TB_closeWindowButton' title='Закрыть гелерею'>Закрыть</a></div>");
