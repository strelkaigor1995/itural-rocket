			...
			<div id="wpsc-col-right">			
				<div id='poststuff' class="col-wrap">
					...
					<?php
						...
						if($product_id != 0) wpsc_display_product_form($product_id);
					?>
					</form>
				</div>
			</div>
						...
						?>
							<tr class="product-edit <?php echo ( wpsc_publish_status($product['id']) ) ? ' wpsc_published' : ' wpsc_not_published'; ?>" id="product-<?php echo $product['id']?>" >
									...
									<td class="product-title column-title">
									  ...
									  ?>
										<?php if($product['publish'] != 1 ) { ?><span style="font-size:.8em;">[Черновик]</span><?php } ?>
										<a style="font-size:.8em;" class='edit-product' href='<?php echo $edit_product_url; ?>' title="ID #<?php echo $product['id']; ?>: <?php echo $product_name; ?>"><?php echo $product_name; ?></a>											
											<?php
