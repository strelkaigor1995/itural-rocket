...
function wpsc_ajax_staff_select() {
	global $wpdb;
  	$staff_id = absint($_REQUEST['id_staff']);
  	$sql = "select id, family from compat_families where maker_id =".$staff_id." ORDER BY family ASC";
  	$staff_value = $wpdb->get_results($sql);

  	die(json_encode($staff_value));
}

if($_REQUEST['wpsc_admin_action'] == 'staff_select') {
	add_action('admin_init', 'wpsc_ajax_staff_select');
}
...
function wpsc_ajax_load_product() {
  ...
  check_admin_referer('edit_product_' .  $product_id);
  	if($page == 'wpsc-staff'){
		wpsc_display_product_form_staff($product_id);
	} else if($page = 'wpsc-edit-products') {
...
function wpsc_swfupload_images() {
	...
	// нужна права 'edit_posts', это Участник и выше как и в \wp-content\plugins\wp-e-commerce\wpsc-admin\admin.php
...
function wpsc_admin_submit_staff(){
	global $wpdb;
	$desc = $_POST['content'];
	$img = $_POST['image'];
	$product_id = $_POST['product_id'];
	
	$sql = "UPDATE compat_devices SET image = ".$img." AND description = ".$desc." WHERE id = ".$product_id;
	$wpdb->query($sql);
} 
...
if($_GET['display_invoice']=='true') {
 ...
 if($_REQUEST['wpsc_admin_action'] == 'edit_staff') {
	add_action('admin_init', 'wpsc_admin_submit_staff');
}
