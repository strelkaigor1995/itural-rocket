...
function wpsc_display_edit_products_staff_page() {
  global $wpdb;
	
	$category_id = absint($_GET['category_id']);

	$series =  (isset($_GET['series'])) ? absint($_GET['series']) : '';
	$producer = (isset($_GET['producer'])) ? absint($_GET['producer']) : '';
	...
	?>
		...
		<div id="col-container">
			<div id="wpsc-col-right">			
				<div id='poststuff' class="col-wrap">
					...
					<?php
						...
						if($product_id != 0) wpsc_display_product_form_staff($product_id);
					?>
					</form>
				</div>
			</div>
			...
			<div id="wpsc-col-left">
				<div class="col-wrap">		
					<?php
						wpsc_admin_products_list_staff($producer, $series);
					?>
				</div>
			</div>
		</div>
...
function wpsc_admin_products_list_staff($producer = 0, $series = 0) {
  ...
  $search_input = '';
	...
	if($producer > 0) {  
		...
		$sql = "SELECT c_m.maker 		AS maker,
		 		...
		 		AND c_d.active 		  = 1
		 		AND c_d.family_id 	= {$series}

				ORDER BY c_d.name ASC";	  					    
	...
	$num_products = $wpdb->get_var("SELECT COUNT(DISTINCT `id`) FROM `compat_devices` WHERE `active`='1' $search_sql");
	...
	<div class="wpsc-separator"><br/></div>
		...
		<?php $producers = wpsc_admin_producer_dropdown_staff(); ?>
		<div class="alignleft actions">
			<form action="admin.php" method="get">
				<input type='hidden' name='page' value='<?php echo $_GET['page'] ?>' />
				<select name="producer" id="producer">
					<option value="0" selected>Выбрать производителя</option>
					<?php foreach ($producers as $producer): ?>
						<option value="<?php echo $producer['id']; ?>"><?php echo $producer['maker']; ?></option>
					<?php endforeach; ?>
				</select>

				<select style="width:200px;" name="series" id="series">
						<option></option>
				</select>
		...
		<div class="tablenav">	
			...
			<div class="alignleft actions">
					<select name="bulkAction">
						...
						<option value="show">Опубликовать</option>
						<option value="hide">В черновик</option>
			...
			<tbody>
				...
				if(count($product_list) > 0) {
					...
					foreach((array)$category_list as $category_row) {
						...
						$category_html .= "<a style=\"font-size:.8em;\" class='category_link' href='". htmlentities(remove_query_arg('product_id',add_query_arg('category_id', $category_row['id'])))."'>".stripslashes($category_row['name'])."</a>";
						$i++;
					}        
							...
							<tr class="product-edit <?php echo ( wpsc_publish_status($product['id']) ) ? ' wpsc_published' : ' wpsc_not_published'; ?>" id="product-<?php echo $product['id']?>" >
									...
									<td class="product-image ">
										...
										<?php if(empty($product['image'])): ?>
											<img title='Drag to a new position' src='<?php echo $image_path; ?>'
												 alt='<?php echo $product['name']; ?>' width='38' height='38' />
										<?php  else: ?>
											<?php $img_staff_path = home_url(). '/wp-content/plugins/wp-e-commerce/images/staff/'; ?>
											<img title='Drag to a new position' src='<?php echo $img_staff_path.$product['image']; ?>'
												 alt='<?php echo $product['name']; ?>' width='38' height='38' />		 
										<?php endif; ?>	
									...
									<td class="product-title column-title">
									  ...
									  ?>
										...
										<a style="font-size:.8em;" class='edit-product' href='<?php echo $edit_product_url; ?>' title="ID #<?php echo $product['id']; ?>: <?php echo $product_name; ?>"><?php echo $product['maker']." ".$product['family']." ".$product_name; ?></a>											
											<?php
										...
										<div class="wpsc-row-actions">
											<span class="edit">
												<a class='edit-product' data-page="<?php echo $_GET['page']; ?>" href='<?php echo $edit_product_url; ?>' style="cursor:pointer;">Редактировать</a>
											</span>
											<!-- |
											<span class="delete">
											 ...
											 |
											<span class="view">
												<a target="_blank" rel="permalink" href="<?php echo wpsc_product_url($product['id']); ?>">Просмотреть</a>
											</span>
											<!--|
											<span class="view">
												<a rel="permalink"
											...
											<span class="publish_toggle">
												<a href="<?php echo wp_nonce_url("admin.php?wpsc_admin_action=toggle_publish&product=".$product['id'], 'toggle_publish_'.$product['id']); ?>"
													>
													<?php
									...
									<td class="product-price column-price">
										<span class="pricedisplay" style="font-size:.8em;"><?php echo nzshpcrt_currency_display($product['price'], 1, true); ?></span>
...
function wpsc_admin_producer_dropdown_staff() {
	global $wpdb;

	$table_name = "compat_makers";
	$sql = "SELECT * FROM ". $table_name . " ORDER BY maker ASC";
...
if(isset($_POST['staff_submit'])){
	...
	$img = $_FILES['my_image_upload'];

	if(isset($img) && !empty($img['name'])){
	
		$file_dir = ABSPATH . 'wp-content/plugins/wp-e-commerce/images/staff/';
		$im_path = ABSPATH . 'wp-content/uploads/2018/3/';

		$result = upload_file($img, $file_dir);
	    ...
	    if(!isset($error)){
			$sql = "UPDATE `compat_devices` SET `image` = '".$img."', `description` = '".$desc."' WHERE `id` = '".$product_id."'";
			$result = $wpdb->query($sql);
			if($result === false) die("Ошибка БД");
		} else {
			die($error);
		}
	} else {
		$sql = "UPDATE `compat_devices` SET `description` = '".$desc."' WHERE `id` = '".$product_id."'";
		$result = $wpdb->query($sql);
		if($result === false) die("Ошибка БД");
	} 
}
