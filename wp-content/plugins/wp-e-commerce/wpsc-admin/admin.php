...
require_once(WPSC_FILE_PATH."/wpsc-admin/display-staff.page.php");
...
require_once(WPSC_FILE_PATH."/wpsc-admin/includes/display-staff-functions.php");
...
function wpsc_admin_pages(){
  ...
  global $userdata;
    ...
    if(function_exists('add_options_page')) {
			...
			$base_page = 'wpsc-edit-products';
			
			//exit(var_dump($userdata->user_level));
			//exit(var_dump($userdata));
			
			//exit(var_dump($userdata->has_cap('administrator')));
			...
			/*if ($userdata->user_level <= 2) {
				...
				//exit(var_dump(function_exists('add_object_page')));
				/*if (function_exists('add_object_page')) {
					add_object_page(__('Store', 'wpsc'), __('Store', 'wpsc'), 2, $base_page, array(), WPSC_URL."/images/credit_cards.png"); // deprecated
				} else {
					add_menu_page(__('Store', 'wpsc'), __('Store', 'wpsc'), 2, $base_page, array(), WPSC_URL . '/images/credit_cards.png');
				//}
			}*/

			// нужна права 'edit_posts', это Участник и выше
			add_menu_page(__('Store', 'wpsc'), __('Store', 'wpsc'), 'edit_posts', $base_page, array(), WPSC_URL . '/images/credit_cards.png');
			...
			// нужна права 'edit_posts', это Участник и выше
			// еще проверяются права на загрузку изображений \wp-content\plugins\wp-e-commerce\wpsc-admin\ajax-and-init.php
			$edit_products_page = add_submenu_page($base_page,__('Products', 'wpsc'),__('Products', 'wpsc'), 'edit_posts', 'wpsc-edit-products', 'wpsc_display_edit_products_page');

			$page_hooks[] = $edit_products_page;

			$page_hooks[] = add_submenu_page($base_page, "Аппараты", "Аппараты", 'manage_options', 'wpsc-staff', 'wpsc_display_edit_products_staff_page');

			//echo $edit_products_page;
			
			// нужна права 'edit_posts', это Участник и выше
			$page_hooks[] = add_submenu_page($base_page,__('Categories', 'wpsc'), __('Categories', 'wpsc'), 'edit_posts', 'wpsc-edit-groups', 'wpsc_display_groups_page');
...
//			exit($GLOBALS['wp_version']);
			...
			$edit_options_page = add_submenu_page($base_page,__('Settings', 'wpsc'), __('Settings', 'wpsc'), 'manage_options', 'wpsc-settings', 'wpsc_display_settings_page');
			$page_hooks[] = $edit_options_page;

			$page_hooks[] = add_submenu_page($base_page,__('Upgrades', 'wpsc'), __('Upgrades', 'wpsc'), 'manage_options', 'wpsc-upgrades', 'wpsc_display_upgrades_page');
...
function  wpsc_admin_include_css_and_js() {
  ...
  $version_identifier = WPSC_VERSION.".".WPSC_MINOR_VERSION;
	...
	if(defined('WPSC_GOLD_DIR_NAME') && WPSC_GOLD_DIR_NAME != ''){
		wp_enqueue_style('gold_cart', '/wp-content/uploads/wpsc/upgrades/gold_cart_files/gold_cart.css',false, $version_identifier, 'all');
	}
