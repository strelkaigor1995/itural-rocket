...
function wpsc_admin_submit_product() {
  ...
  if((int)$post_data['product_id'] != 0){
		...
		if($product_id > 0) { $sendback = add_query_arg('product_id', $product_id);	}
	...
	$_SESSION['product_error_messages'] = array();
  	...
  	if((int)$post_data['product_id'] == 0){
  		$_SESSION['product_error_messages'][] = 'Новые товары добавляются только автоматически из 1С';
  	}
...
//return;
  ...
  foreach($product_columns as $column => $default) {
   ...
   if($update === true) {
	...
	// add url_name from id
	//update_product_meta($product_id, 'url_name', $product_id);
...
//print_r($categories);
//print_r($associated_categories);
//print_r($categories_to_add);
//print_r($categories_to_delete);

  foreach($categories_to_add as $category_id) {
    $insert_sections[] = $wpdb->prepare("( %d, %d)", $product_id, $category_id);
  }

//echo count($insert_sections) . "\n";
