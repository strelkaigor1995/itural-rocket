  ...
  // this loads the edit-products page using javascript
	 jQuery('.edit-product').click(function(){	
			...
			wpnonce = jQuery(this).attr('href').match(/_wpnonce=(\w{1,})/);
	 		post_values = "product_id="+product_id[1]+"&_wpnonce="+wpnonce[1]+"&page="+page;
	 ...
	 jQuery('#producer').change(function(){
	 	...
	 	var values = "id_staff="+id
	 	jQuery('#series').empty();
	 	jQuery.post( 'index.php?wpsc_admin_action=staff_select', values, function(returned_data) {
	 		var data = JSON.parse(returned_data)
	 		for (var staff_id in data){
	 			jQuery('#series').append("<option value='"+data[staff_id]['id']+"'>"+ data[staff_id]['family'] +"</option>");
	 		}
	 	});
	 });
...
function wpsc_edToolbar() {
	...
	//console.log(edButtons.length);
	
	for (i = 0; i < edButtons.length; i++) {
		
		if(edButtons[i] == undefined) continue;
		//console.log(edButtons[i]);
