...
Description: ***** НЕ ОБНОВЛЯТЬ ***** <a href="http://getshopped.org" target="_blank">GetShopped.org</a> | <a href="http://getshopped.org/forums/" target="_blank">Support Forum</a> | <a href="http://getshopped.org/resources/docs/" target="_blank">Documentation</a>
Version: 3.7.6.9
Author: Instinct Entertainment & ]aka[
...
// дев-дебаг дефайны здесь, чтобы грузить до темы
//define('DEVELOPMENT', true);

if(defined('DEVELOPMENT')){
	
	define('MY_COPY_ALERT', true);

	define('MY_JIVOSITE', true);
	define('MY_RETAIL_ROCKET', false);

	define('MY_FEEDBACK_EMAIL', 'cio@itural.ru');
	
	define('MY_MSSQL_ADDRESS', '192.168.0.254');
	
	define('MY_CALLBACK', true);
	define('MY_CALLBACK_EMAIL', 'cio@itural.ru');
	define('MY_AST_ADDRESS', '192.168.0.245');
	define('MY_AST_DID', 'local/999@from-trunk');
	
} else {

	define('MY_COPY_ALERT', false);

	define('MY_JIVOSITE', true);	
	define('MY_RETAIL_ROCKET', false);

	define('MY_FEEDBACK_EMAIL', 'mail@itural.ru');
	
	define('MY_MSSQL_ADDRESS', 'hq.itural.ru');
	
	define('MY_CALLBACK', true);
	define('MY_CALLBACK_EMAIL', 'callback@itural.ru');
	define('MY_AST_ADDRESS', 'hq.itural.ru');
	define('MY_AST_DID', 'local/001@from-trunk');
...
//exit(var_dump($wp_upload_dir_data));
...
// move from uploads to plugin dir
//$wpsc_upgrades_dir = "{$wpsc_upload_dir}upgrades/";
$wpsc_upgrades_dir = WPSC_FILE_PATH . '/upgrades/';

// move from uploads to plugin dir
//$wpsc_themes_dir = "{$wpsc_upload_dir}themes/";
$wpsc_themes_dir = WPSC_FILE_PATH . '/themes/';
...
$wpsc_upgrades_url = WPSC_URL . '/upgrades/';

// move from uploads to plugin dir
//$wpsc_themes_url = "{$wpsc_upload_url}themes/";
$wpsc_themes_url = WPSC_URL . '/themes/';
...
// jakal, first referer
if(!isset($_SESSION['first_referer'])){
	$_SESSION['first_referer'] = $_SERVER['HTTP_REFERER'];
}
