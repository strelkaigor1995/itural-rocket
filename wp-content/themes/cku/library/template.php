...
function arras_alternate_style() {
	...
	if (ARRAS_CHILD && count($arras_registered_alt_styles) > 0) {
		...
		if ( $scheme != 'default' )
			echo '
<link rel="stylesheet" href="' . get_bloginfo('stylesheet_directory') . '/css/styles/' . $scheme . '.css" type="text/css" media="all" />
			';
		else
			echo '
<link rel="stylesheet" href="' . get_bloginfo('stylesheet_directory') . '/css/styles/default.css" type="text/css" media="all" />
			';
		
		//if (!ARRAS_CHILD) { echo '<link rel="stylesheet" href="' . get_bloginfo('template_url') . '/css/user.css" type="text/css" media="all" />';}
	}
...
<?php elseif (arras_get_option($page_type . '_news_display') == 'line') : ?>
	...
	<li <?php arras_post_class() ?>>
		...
		<a rel="bookmark" href="<?php the_permalink() ?>" title="<?php printf( __('Permalink to %s', 'arras'), get_the_title() ) ?>"><?php the_title() ?></a> <?php /*"*/ ?>
		<span class="entry-comments"><?php comments_number('','1','%') ?></span>
	</li>
	<?php endwhile; ?>
	</ul>
<?php else : ?>
...
?>
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
		...
		<div class="comment-author vcard">
		    ...
		    <?php print( '<span class="published" title="' . get_comment_time( get_option('date_format').', '.get_option('time_format') ) . '">' . get_comment_time( get_option('date_format').', '.get_option('time_format') ) . '</span>' ); ?>
		</div>
