...
function arras_newsfooter($page_type) {
	...
	/* comments number */
	$postfooter .= '<span class="entry-meta">';
	if (get_comments_number()) $postfooter .= '<a href="' . get_permalink() . '"><span class="entry-comments">' . get_comments_number() . '</span></a>';

	/* date */
	if ( ! is_home() ) {
	    $postfooter .= '    <abbr class="published" title="' . get_the_time(get_option('date_format').' '.get_option('time_format')) . '">' . get_the_time( get_option('date_format') ) . '</abbr>';
	}    
	$postfooter .= '</span>';
...
function arras_postheader() {
	...
	/* print("single:".is_single()." page:".is_page()." attach:".is_attachment()); */
...
function arras_postfooter() {
	...
	if ( !is_page() ) {
		...
		$postfooter .= '<div class="entry-info">';
		
		if ( arras_get_option('post_author') ) {
			$postfooter .= sprintf( __('<span class="entry-author">By %s</span>', 'arras'), '<address class="author vcard">' . get_the_author() . '</address>' );
		}
		
		if ( arras_get_option('post_date') ) {
			$postfooter .= '<abbr class="published" title="' . get_the_time( get_option('date_format') ) . ', ' . get_the_time( get_option('time_format') ) . '">' . sprintf( __('<strong>Published:</strong> %s', 'arras'), get_the_time( get_option('date_format') ) . ', ' . get_the_time( get_option('time_format') )) . '</abbr>';
		}
		...
		if ( arras_get_option('post_cats') ) {
			...
			$postfooter .= sprintf( __('<span class="entry-cat"><strong>Posted in: </strong>%s</span>', 'arras'), implode(', ', $post_cats) );
		}
		
		if ( arras_get_option('post_tags') && !is_attachment() )
			if ( has_tag() ) { $postfooter .= '<span class="tags clearfix">' . __('Tags:', 'arras') . ' ' . get_the_tag_list('', ', ', '') . '</span>'; }
...
function arras_posttags() {
	if ( has_tag() ) {
		$postfooter .= '<div class="entry-info"><span class="tags clearfix">' . __('Tags:', 'arras') . ' ' . get_the_tag_list('', ', ', '') . '</span></div>';
		echo apply_filters('arras_postfooter', $postfooter);
	}
}
